...
class _WorkspacePageState extends State<WorkspacePage> {
  ...
  Future<void> _createDatabase() async {
    final name = await WorkspaceDialogs.showForm(
      context: context,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: const Color(0xFFB06A60),
          ),
        );
      }
    }
  ...
  Future<void> _renameDatabase(String oldDb) async {
    final newDb = await WorkspaceDialogs.showForm(
      context: context,
  ...
  Future<void> _dropDatabase(String db) async {
    final confirm = await WorkspaceDialogs.showConfirm(
      context: context,
      title: "Drop Database",
      message:
          "Are you sure you want to drop database '$db'? This action cannot be undone.",
      confirmText: "DROP",
    );
  ...
  Future<void> _cloneTable(String db, String table) async {
    final result = await WorkspaceDialogs.showCloneTable(
      context: context,
      table: table,
    );
  ...
  Future<void> _truncateTable(String db, String table) async {
    final disableFk = await WorkspaceDialogs.showConfirm(
      context: context,
      title: "Truncate Table",
      message:
          "Are you sure you want to truncate table '$table'? All data will be lost.",
      showFkCheckbox: true,
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Table truncated successfully."),
  ...
  Future<void> _dropTable(String db, String table) async {
    final disableFk = await WorkspaceDialogs.showConfirm(
      ...
      title: "Drop Table",
      message:
          "Are you sure you want to drop table '$table'? This action cannot be undone.",
      showFkCheckbox: true,
      confirmText: "DROP",
    );
    if (disableFk == null) return;
    ...
    return CallbackShortcuts(
      ...
      child: Scaffold(
        appBar: AppBar(
            ...
            children: [
              ...
              ValueListenableBuilder<double>(
                ...
                builder: (context, width, child) {
                  return IconButton(
                    icon: Icon(
                      width > 0 ? Icons.menu_open : Icons.menu,
                      size: 20,
                    ),
                    onPressed: () {
                      if (width > 0) {
          ...
          actions: [
            ValueListenableBuilder<WorkspaceConnectionState>(
              valueListenable: _controller.connectionState,
        ...
        body: Row(
          children: [
            ValueListenableBuilder<double>(
              ...
              child: ValueListenableBuilder<WorkspaceSchemaState>(
                valueListenable: _controller.schemaState,
                builder: (context, state, child) {
                  return Container(
            ...
            MouseRegion(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Container(width: 1, color: const Color(0xFFE8E5DF)),
                  ),
                ),
              ),
            ),
            Expanded(
              child: ValueListenableBuilder<WorkspaceTabsState>(
                valueListenable: _controller.tabsState,
                builder: (context, state, child) {
                  return Column(
                    children: [
                      WorkspaceTabsBar(
  ...
  Widget _buildConnectionStatus(WorkspaceConnectionState state) {
    return Center(
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 16),
        child: Row(
          children: [
            _BreathingConnectionDot(isAlive: state.isAlive),
            const SizedBox(width: 8),
            Text(
              state.isAlive ? "Connected" : "Offline",
              style: TextStyle(
                fontSize: 12,
            ...
            if (!state.isAlive) ...[
              const SizedBox(width: 8),
              TextButton(
                onPressed: state.isChecking ? null : _controller.reconnect,
  ...
  Widget _buildActiveTabContent(WorkspaceTabsState state) {
...
class _BreathingConnectionDot extends StatefulWidget {
  ...
  @override
  State<_BreathingConnectionDot> createState() =>
      _BreathingConnectionDotState();
}

class _BreathingConnectionDotState extends State<_BreathingConnectionDot>
